...
Owns the app-server process and its one thread (new and ephemeral on every launch).
Turns run on a worker thread, which is the only reader of the app-server after setup.
Every protocol message is journaled as in task 2 (capabilities a-e); what a person
needs to see is also published as small typed events on an EventLog, which ui.py
streams to the browser. A new kind of event = one publish() here plus one renderer in
static/app.js (unknown types are still shown, generically).
"""
...
CLIENT_INFO = {
    "name": "nimoi_claude_codex_ui",
    "title": "NIMOI bootstrap-harness claude-codex task-3-ui",
    "version": "0.1.0",
}
...
TURN_TIMEOUT = 600
ALLOWED_CALLS = {spec["name"] for spec in tools.SPECS} | set(policy.REVIEWED_MODEL_TOOLS)
...
def fake_reply(body):
    """Scripted stand-in model for --fake-model: calls add when asked to add two numbers,
    reports its result, and otherwise echoes. It never calls any other tool."""
    ...
    if last.get("type") == "function_call_output":
        ...
        try:
            return [fake_model.message(f"The add tool says the sum is {json.loads(output)['sum']}.")]
        except (ValueError, KeyError, TypeError):
            return [fake_model.message(f"The add tool returned: {output}")]
    ...
    if "add" in text.lower() and len(numbers) >= 2:
        a, b = (float(n) if "." in n else int(n) for n in numbers[:2])
        return [fake_model.function_call("call_fake_add", "add", {"a": a, "b": b})]
...
class Conversation:
    ...
    def __init__(self, codex=None, codex_home=None, model=None, fake=False, runs_dir=RUNS_DIR):
        self.events = EventLog()
        self.journal = cc.Journal(runs_dir, "ui-fake" if fake else "ui")
    ...
    def start(self):
        ...
        self.journal.write("run", harness="task-3-ui", model=self.model, fake_model=self.fake,
                           codex_home=env["CODEX_HOME"],
                           disabled_features=policy.DISABLED_FEATURES,
                           thread_params=policy.THREAD_PARAMS, tools=tools.SPECS)
        ...
        self.events.publish(
            ...
            tools=[spec["name"] for spec in tools.SPECS],
    ...
    def _work(self):
        while True:
            ...
            try:
                ...
                self.journal.write("error", error=repr(error), traceback=traceback.format_exc())
    ...
    def _on_notification(self, message):
        ...
        if method == "thread/tokenUsage/updated":
            ...
            if name is not None:
                reviewed = name in ALLOWED_CALLS
    ...
    def _on_model_request(self, entry):
        ...
        if not self._offered_published:
            ...
            unreviewed = [n for n in names if n not in ALLOWED_CALLS]
