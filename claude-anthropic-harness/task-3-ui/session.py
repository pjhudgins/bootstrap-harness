"""One agent conversation, owned by one long-lived asyncio task.

The worker task enters and exits the ClaudeSDKClient context itself, because
the SDK's anyio task group must be entered and exited by the same task. Web
requests never touch the client for a turn; they drop text into the inbox.
The only cross-task call is interrupt().

Capabilities carried over from task 2 (see task-2-tooling/driver.py):
  a. every stream message published with direction      b. policy.py, via hook + can_use_tool
  c. calc_tool.py add over in-process MCP                d. tool_call / permission records
  e. usage, rate_limit, mcp_status, context_usage, account (type + provider only)
...
class AgentSession:
    def __init__(self, bus: EventBus, *, model: str, workspace: Path, budget_usd: float, max_turns: int):
        self.bus = bus
        self.model = model
        ...
        # per-turn (verified live 2026-09-23: turn 2 model_usage = turn 1 + turn 2 tokens).
        # Only ResultMessage.usage is per-turn.
    ...
    async def start(self, timeout: float = 90) -> None:
        ...
        if self.state == "failed":
            raise RuntimeError("agent session failed to start; see run log")
    ...
    async def _run(self) -> None:
        try:
            async with ClaudeSDKClient(options=self._options()) as client:
                ...
                self.bus.publish(
                    "session_start", model=self.model, cwd=str(self.workspace),
                    loaded_builtins=ALLOWED_BUILTINS, mcp_servers=[SERVER_NAME],
    ...
    def _options(self) -> ClaudeAgentOptions:
        ...
        return ClaudeAgentOptions(
            ...
            strict_mcp_config=True,  # keep account claude.ai connectors out (task-2 finding 1)
