// task-3-ui front end. Every record from /api/events is one line of the run log.
...
// One side-panel line: seq, kind, summary; click to expand the full record.
function side(rec, summary, cls = "") {
  const d = el("details", "ev " + cls);
  const s = el("summary");
  s.append(el("span", "seq", rec.seq), el("span", "kind", rec.kind), el("span", "text", summary));
  d.append(s);
...
const toolLabel = (name) => (name || "").replace(/^mcp__calc__/, "");
...
const renderers = {
  ...
  status(rec) {
    ...
    input.disabled = !idle; $("send").disabled = !idle; $("stop").disabled = rec.state !== "busy";
    if (idle) input.focus();
    if (rec.state === "over_budget") chat("bubble error", `Budget of $${Number(rec.budget_usd).toFixed(2)} reached. Relaunch for a new conversation.`);
  ...
  session_start(rec) {
    ...
    side(rec, `${rec.model} · tools ${rec.loaded_builtins.join(", ")} + ${rec.mcp_servers.join(", ")} · budget $${rec.budget_usd}`);
  },

  session_tools(rec) { side(rec, (rec.tools || []).join(", ")); },
  ...
  tool_call(rec) {
    ...
    else if (rec.phase === "post") side(rec, `${toolLabel(rec.tool_name)} → ${short(rec.tool_response)}`, "allow");
...
function render(rec) {
  ...
  seen.add(rec.seq);
  (renderers[rec.kind] || ((r) => side(r, short(Object.fromEntries(Object.entries(r).filter(([k]) => !["seq", "ts", "kind", "turn"].includes(k)))))))(rec);
}
...
source.onerror = () => {
  const b = $("state");
  if (source.readyState !== EventSource.OPEN) { b.textContent = "disconnected"; b.className = "badge disconnected"; input.disabled = true; $("send").disabled = true; }
};
