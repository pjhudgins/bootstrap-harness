"""Launch a ledger-backed local UI and a fresh Codex test-pilot conversation."""
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/api/ledger":
                return self.reply(200, conversation.journal.bytes(),
...
def main():
    ...
    try:
        ...
        conversation.record_if_available("launch_failed", {"error": "Unable to bind local port."})
        conversation.journal.close()
        raise
    conversation.start()
    print(f"NIMOI: {server.origin}", flush=True)
    print(f"Ledger: {conversation.log_path}", flush=True)
