...
function renderTools(tools) {
  if (toolCount === tools.length) return;
  ...
  for (const tool of tools) {
    ...
    if (tool.tool === "add") { node.append(title, result); }
    else {
      const details = document.createElement("details"), summary = document.createElement("summary");
      summary.textContent = title.textContent;
      result.textContent = JSON.stringify({arguments:tool.arguments, output:tool.output}, null, 2).slice(0, 4000);
      details.append(summary, result); node.append(details);
    }
    target.append(node);
  }
}
...
$("end-session").addEventListener("click", async () => {
  ...
  try {
    ...
    error("Session ended. Your ledger is saved locally. Relaunch the driver for a new conversation.");
